...
namespace NhnCloud.GamebaseTools.SettingTool
{
    public class Version
    {
        ...
        public void CheckGamebaseSdkStatus()
        {
            ...
            if ((CheckUpdateRequired(GamebaseInfo.GetCurrentVersion(SupportPlatform.UNITY), data.unity.newest) == true) ||
                (CheckUpdateRequired(GamebaseInfo.GetCurrentVersion(SupportPlatform.ANDROID), data.android.newest) == true) ||
                (CheckUpdateRequired(GamebaseInfo.GetCurrentVersion(SupportPlatform.IOS), data.ios.newest) == true))
            {
            ...
            {
                if (string.IsNullOrEmpty(GamebaseInfo.GetCurrentVersion(SupportPlatform.UNITY)) == false)
                {
                    if (DataManager.GetData<bool>(DataKey.HAS_GAMEBASE_SDK) == true)
                    {
