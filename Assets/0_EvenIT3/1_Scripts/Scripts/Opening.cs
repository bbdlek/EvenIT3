...
public class Opening : MonoBehaviour
{
    ...
    IEnumerator Text()
    {
        yield return StartCoroutine(NormalChat("나래이션", "오래전... 이 학교에는 아이들을 충치로 고통받게 만드는 충치 요정이 살고 있었다는 전설이 내려오고 있는데...", 0));
        yield return StartCoroutine(NormalChat("나래이션", "몇십 년 후…", 2));
        yield return StartCoroutine(NormalChat("나래이션", "땡땡 년도 어느 날 지각한 벌로 창고 청소를 하던 주인공... 실수로 항아리를 깨버렸다", 2));
        MasterAudio.PlaySound("easy opening_2");
        yield return StartCoroutine(NormalChat("나래이션", "선생님에게 혼나기 싫었던 주인공은 급하게 깨진 조각들을 버리고 집으로 돌아갔다.", 2));
        ...
        yield return StartCoroutine(NormalChat("주인공", "어제 덜 치운 항아리 조각이 있는지 확인해 봐야겠다.", 9));
        yield return StartCoroutine(NormalChat("빛덩어리", "야아아아아아아아아아아아아아아아아아", 10));
        yield return StartCoroutine(NormalChat("주인공", "아아아아아아아아앙ㄱ 뭐야!? (벌레 쫓듯이 손을 휘적인다)", 11));
        yield return StartCoroutine(NormalChat("건치 요정", "잠깐!!!!!!!!! 진정해!!!!!!!!!!!", 12));
        yield return StartCoroutine(NormalChat("주인공", "뭐야뭐야뭐야", 13));
        yield return StartCoroutine(NormalChat("건치 요정", "진정하라고오오오오오! 나는 건치 요정이라고!!! 나는 어제 네가 깬 항아리 때문에 잘 자고 있었는데 잠에서 깼어 !!!\n그 항아리가 어떤 항아리인 줄 알아??? 아주 못된 충치 요정을 봉인했던 항아리라고!!!\n지금 너 때문에 온 학교 전교생이 충치가 생기고 있단 말이야!!!", 14));
