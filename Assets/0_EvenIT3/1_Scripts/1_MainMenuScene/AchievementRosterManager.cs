    ...
    private void InitRosterUI()
    {
        switch (Type)
        {
            case 0: // N 스테이지 최초 클리어
                _headerText.text = (No + 1) + "스테이지 최초 클리어";
                break;
            case 1: // N챕터의 모든 스테이지를  한번도 실패하지 않고 연속 클리어
                _headerText.text = (No - 23) + "챕터의 모든 스테이지를 한번도 실패하지 않고 연속 클리어";
                break;
            case 2: // 돌아보기로 걸린 횟수 N회
                _headerText.text = NN + "돌아보기로 실패한 횟수 " + NN + "회";
                break;
            case 3: // 제한시간 만료로 실패한 횟수 N회
                _headerText.text = NN + "제한시간 만료로 실패한 횟수 " + NN + "회";
                break;
            case 4: // 데시벨 초과로 실패한 횟수 N회
                _headerText.text = NN + "데시벨 초과로 실패한 횟수 " + NN + "회";
                break;
            case 5: // 아이템a 사용 횟수 N회
                _headerText.text = NN + "무료아이템 사용 횟수 " + NN + "회";
                break;
            case 6: // 아이템A 사용 횟수 N회
                _headerText.text = NN + "유료아이템 사용 횟수 " + NN + "회";
                break;
            case 7: // 누적으로 먹기 성공한 간식 개수 N개
                _headerText.text = NN + "누적으로 먹기 성공한 간식 개수 " + NN + "개";
                break;
            case 8: // 부적 N개 획득(10개 단위)
                _headerText.text = NN + "부적 " + NN + "개 획득";
                break;
            case 9: // 최종 엔딩 완료
                _headerText.text = NN + "최종 엔딩 환료";
                break;
            case 10: // 일반 뽑기 누적 N회
                _headerText.text = NN + "일반 뽑기 누적 " + NN + "회";
                break;
            case 11: // 고급 뽑기 누적 N회
                _headerText.text = NN + "고급 뽑기 누적 " + NN + "회";
                break;
        }
